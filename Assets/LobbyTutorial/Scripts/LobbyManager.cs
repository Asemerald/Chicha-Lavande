...
public class LobbyManager : MonoBehaviour {
    ...
    private void Awake() {
        Instance = this;
        OnGameStarted += HideAllUI_OnGameStarted;
    }

    private void HideAllUI_OnGameStarted(object sender, EventArgs e)
    {
    ...
    private async void HandleLobbyPolling() {
        if (joinedLobby != null) {
            ...
            if (lobbyPollTimer < 0f) {
                ...
                if (joinedLobby.Data.TryGetValue("RelayCode", out var value))
                {
                    joinedLobby = null;
                    OnGameStarted?.Invoke(this, EventArgs.Empty);
                    
                    if (IsLobbyHost()) return; // Host doesn't need to join the relay
                    string relayCode = value.Value;
                    Debug.Log("Relay Code: " + relayCode);
                    RelayManager.Instance.JoinRelayAsync(relayCode);
                }
            }
        }
    }
